...
biz:
  oss:
    ...
    resources-url: http://192.168.1.46:9000/shop4cloud
    type: 1
...
1. `shop4cloud-admin`、`shop4cloud-platform` 对于这两个项目修改根目录下的`.env.{环境}`相关文件，如开发环境修改`.env.development`文件。

- VUE_APP_RESOURCES_URL:  对应上面后台配置的resources-url
- VUE_APP_RESOURCES_TYPE： 对应上面后台配置的type

2. `shop4cloud-uniapp` 这个项目修改 `src/utils/config.js`
