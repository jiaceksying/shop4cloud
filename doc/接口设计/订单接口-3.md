...
前端调用接口为`/shop4cloud_payment/pay/order`，根据订单号进行支付，上一步提交订单返回的便是订单id列表
...
public ServerResponseEntity<?> pay(HttpServletRequest request, @Valid @RequestBody PayInfoDTO payParam) {
    ...
    String gatewayUri = "http://192.168.1.17:8126/shop4cloud_payment";
...
public PayInfoBO pay(Long userId, PayInfoDTO payParam) {
    ...
    if (!ordersAmountAndIfNoCancelResponse.isSuccess()) {
        throw new shop4cloudException(ordersAmountAndIfNoCancelResponse.getMsg());
    }
    //将数据存到数据库中
